...
import { TbArrowsShuffle2, TbArrowCapsule } from 'react-icons/tb'
...
const Footer: React.FC = () => {
  ...
  const handleMute = () => {
    ...
    dispatch(
      setVolume({ isMute: !volume.isMute, value: volume.value })
    )
  }
  ...
  const handlePlayForward = () => {
    const newList: PlayListType = sortPlayList(1, waitingList)
  ...
  return (
    <div
      className={`${style.footer} ${!track && style.no__track}`}
    >
      <TrackFooter />
      <div className={style.track__process}>
        ...
        >
          <source src={track?.src?.path} type='audio/mpeg' />
      ...
      <div className={style.track__option}>
        <button hover-content={'Chế độ xem'}>
          <BsFilePlayFill />
        </button>
